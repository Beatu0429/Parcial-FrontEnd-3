...
export default function Item({producto, setCantidad}) {
  ...
  return (
    <div className={estilos.producto}>
      ...
      <h5>En stock: {stock === 0? <span className={estilos.agotado}>Agotado</span>: stock} </h5>
      <button className={stock > 0? estilos.btn:estilos.disabled} onClick={()=> handlerStock()}>{stock > 0? "Comprar":"Sin stock"}</button>
      {/* maquetar Item aquí */}
    </div>
  )
}
